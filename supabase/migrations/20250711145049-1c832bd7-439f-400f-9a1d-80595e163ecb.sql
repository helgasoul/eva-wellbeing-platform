...
DROP POLICY IF EXISTS "Users can view their own lab orders" ON public.lab_orders;
DROP POLICY IF EXISTS "Users can create their own lab orders" ON public.lab_orders;
DROP POLICY IF EXISTS "Users can update their own lab orders" ON public.lab_orders;
...
DROP POLICY IF EXISTS "Admins can manage lab tests catalog" ON public.lab_tests_catalog;
CREATE POLICY "Admins can manage lab tests catalog" ON public.lab_tests_catalog
  FOR ALL USING (has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Admins can manage data mappings" ON public.medical_data_mappings;
CREATE POLICY "Admins can manage data mappings" ON public.medical_data_mappings
  FOR ALL USING (has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Admins can view audit logs" ON public.integration_audit_logs;
CREATE POLICY "Admins can view audit logs" ON public.integration_audit_logs
  FOR SELECT USING (has_role(auth.uid(), 'admin'::app_role));

DROP POLICY IF EXISTS "Admins can manage API configurations" ON public.partner_api_configurations;
