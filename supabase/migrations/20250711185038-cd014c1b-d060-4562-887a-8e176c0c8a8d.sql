-- Phase 1: Medical Digital Twin Database Schema
...
-- Health Patterns Table (for AI-driven pattern recognition)
...
CREATE POLICY "Users can manage own lab results" ON public.lab_results
  FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Users can manage own health patterns" ON public.health_patterns
  FOR ALL USING (auth.uid() = user_id);
...
CREATE INDEX idx_lab_results_user_date ON public.lab_results(user_id, test_date);
...
CREATE INDEX idx_health_patterns_user_active ON public.health_patterns(user_id, is_active);
